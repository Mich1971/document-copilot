...
export function SourcePassageSheet({ citation, onOpenChange }: SourcePassageSheetProps) {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-40 bg-background/40 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        onClick={() => onOpenChange(false)}
      />

      <div
        className={`fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-border bg-background shadow-2xl transition-transform duration-300 ease-in-out sm:max-w-lg ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        role="dialog"
        aria-modal="true"
      >
        <div className="flex items-center justify-between border-b border-border p-4">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h2 className="text-base font-semibold text-foreground">Pasaje fuente</h2>
          </div>
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="space-y-1.5">
              <span className="text-[10px] font-mono font-semibold uppercase tracking-wider text-primary bg-primary/10 px-2 py-0.5 rounded">
                Cita [{citation.citationIndex + 1}]
              </span>
              <h3 className="text-lg font-semibold text-foreground leading-snug">
                {citation.documentTitle}
              </h3>
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-2">
                <span>Indice de Chunk: {citation.chunkIndex}</span>
                <ChevronRight className="h-3 w-3" />
                <span>Documento: {citation.documentId}</span>
              </div>
            </div>
